...
import { GET_TPUP_REQS, GET_WITHDRWL_REQS, USERS_LOADED } from './types';
...
//get users lookup
export const usersLookup = () => async (dispatch) => {
  try {
    const res = await axios.get('/api/auth/users-lookup', API_CONFIG);
    dispatch({
      type: USERS_LOADED,
      payload: res.data,
    });
